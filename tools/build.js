...
const mycss = fse.readFileSync(`${stylesRoot}/index.css`);
...
const copyTypes = (dest) => shell(`cp ${typesRoot}/*.d.ts ${dest}`);
...
/**
 * Bundles a minified and unminified version of buckles including
...
const copyStyles = step('copy styles', () =>
  postcss()
    .use(atImport())
    .use(cssvariables())
    .process(mycss, {
      from: `${stylesRoot}/index.css`,
    })
    .then((result) => {
      fse.mkdirSync(`${libRoot}/styles`);
      fse.writeFileSync(`${libRoot}/styles/index.css`, result.css);
    })
);

const toggleDev = step('toggle dev', () => shell('npm run toggleDev'));
